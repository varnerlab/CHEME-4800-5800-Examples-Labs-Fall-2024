...
function solve(problem::MyLinearProgrammingDualProblemModel)::Dict{String,Any}
    ...
    # initialize -
    results = Dict{String,Any}()
    c = problem.c; # right-hand side vector in the dual problem
    lb = problem.lb; # lower bounds
    ub = problem.ub; # upper bounds
    A = problem.A; # constraint matrix
    b = problem.b; # objective function coefficients in the dual problem
    ...
    @constraints(model, 
        begin
            transpose(A)*y >= c # my material balance constraints
        end
    );
